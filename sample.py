...
os.chdir(r'D:\Downloads\Language of Feelings\A.Python')
...
# Change directory to the Downloads folder on Windows
...
def create_emotion_button(emotion):
    img_path = os.path.join(script_directory, f"{emotion}.png")
    if not os.path.exists(img_path):
        # If image file doesn't exist in the script directory, try in a relative directory
        img_path = f"{emotion}.png"
    img = tk.PhotoImage(file=img_path)  # Load the image
